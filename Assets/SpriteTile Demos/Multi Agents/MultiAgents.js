...
function SetupAgents () {
	...
	for (var i = 0; i < numberOfAgents; i++) {
		...
		agents[i].position = Int2.zero;	// Bottom-left corner of level has a collider, so this forces a new position
...
function MoveAgents () : IEnumerator {
	while (true) {
		for (var i = 0; i < numberOfAgents; i++) {
			...
			// If not surrounded, pick a new random direction if there's a collider in the way (if not, then the current dir remains unchanged)
